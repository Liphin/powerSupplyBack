...
public class FormDataParse {
     ...
     */
    protected Object parseFormDataSingleParam(InterfaceHttpData data) {
        FormDataParse.logger.debug("New FormData Upload SingleParam : " + " \n\rThe data is : " + data + "  End\n");
        try {
            //解析参数类型数据
    ...
    protected HashMap<String, Object> parseFormDataMultiParams(InterfaceHttpData data) {
        FormDataParse.logger.debug("New FormData Upload: MultiParams" + " \n\rThe data is : " + data + "  End\n");
        try {
            //解析参数类型数据
            if (data.getHttpDataType() == InterfaceHttpData.HttpDataType.Attribute) {
                Attribute attribute = (Attribute) data;//用Attribute装载数据
                hashMap.put(attribute.getName(), DataParser.changeDataType(attribute.getValue()));
    ...
    protected Object parseFormDataEntity(InterfaceHttpData data) {
        FormDataParse.logger.debug("New FormData Upload Entity: " + " \n\rThe data is : " + data + "  End\n");
        try {
            //解析参数类型数据
            if (data.getHttpDataType() == InterfaceHttpData.HttpDataType.Attribute) {
                Attribute attribute = (Attribute) data;//用Attribute装载数据
                ...
                while (decoder.hasNext()) {
                    ...
                    if (data != null) {
                        try {
                            switch (type) {
                                case SINGLE_PARAM:
                                    backData = parseFormDataSingleParam(data);
                                    break;
                                case MULTI_PARAMS:
                                    backData = parseFormDataMultiParams(data);
                                    break;
                                case ENTITY:
                                    backData = parseFormDataEntity(data);
                                    break;
                            }
                        } finally {
                            data.release();
